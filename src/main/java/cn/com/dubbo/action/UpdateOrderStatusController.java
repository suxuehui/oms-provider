...
import cn.com.dubbo.service.order.UpdateOrderStatusService;
...
public class UpdateOrderStatusController {
	...
	private Logger logger = Logger.getLogger(UpdateOrderStatusController.class);
	...
	@ResponseBody
	public Map<String, Object> getUpdateOrder(HttpServletRequest request) {
		...
		try {
			...
			if (CheckParamUtil.check(str)) {
				...
				if (StringUtil.isStringNotBlank(startCreated) && StringUtil.isStringNotBlank(endCreated) && StringUtil.isStringNotBlank(pageNo) && StringUtil.isStringNotBlank(venderId)) {
					ToSendModel ext = updateOrderStatusService.getUpdateOrder(pageNo, pageSize, venderId);
					map.put("code", "10000");
					map.put("rows", ext);
				} else {
					map.put("code", "10001");
