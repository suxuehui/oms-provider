...
public class PsOrderServiceImpl implements PsOrderService{
	...
	// 数据准备，服务端提供的配置参数
	private final static String apiId = "e4e0475234a625f4f78ca7477296b396#PROD"; //即上文apiId
	private final static String apiName = "pageQueryB2COrderForOpenApi"; //即上文apiName,订单查询接口
	
	private final static String sellerId = "12202680605";//平安九药网自营店
	
	private final static int pageSize = 90;//一页显示的条目数量
	...
	private static final Logger logger = Logger.getLogger(PsOrderServiceImpl.class);
	...
	public void dealOrder(String startTime,String endTime,boolean flag){
		...
		try {
			...
			if(flag){
				multiChannelOrderBatch = BussinessUtil.multiChannelOrderBatch("5");
			}else{
				multiChannelOrderBatch = BussinessUtil.multiChannelOrderBatch("15");
			}
	...
	private void saveChannelOrderBatch(long multiChannelOrderBatch){
		...
		channelOrderModel.setMultiChannelId(5);//平安
	...
	private void orderSerach(int pageIndex,String method,
		...
		try {
			logger.info(sdf.format(new Date())+" ps 商城 orderSerach...............");
		...
		try {
			...
			if(modelArray!=null && modelArray.size()>0){
				...
				for(int i=0;i<modelArray.size();i++){
					...
					orderModer.setMultiChannelId(5);//来源渠道ID
					bizOrderId = ChannelConstant.CHANNEL_PS+bizOrderId;
...
//						itemsTmp.getLong("itemSkuId");
					...
					if(payItems!=null && payItems.size()>0){
						...
						for(int p=0;p<payItems.size();p++){
							...
							orderModer.setPlatformCreateTime(sdf.format(new Date(payTime*1000)));
							orderModer.setCommitTime(orderModer.getPlatformCreateTime());
