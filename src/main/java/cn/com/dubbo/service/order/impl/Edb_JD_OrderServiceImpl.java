...
import cn.com.dubbo.service.order.Edb_JD_OrderService;
...
//从e店宝同步JD的订单
@Service("edbJdOrderService")
//@Transactional
public class Edb_JD_OrderServiceImpl implements Edb_JD_OrderService{	
	...
	private static final Logger logger = Logger.getLogger(Edb_JD_OrderServiceImpl.class);
	//--------------------------------------------------------------------------------------
...
//		apiparamsMap.put("end_time", "2016-07-06 13:38:40");
		...
		apiparamsMap.put("shopid", "2");//shopid:店铺id   "shopid": "1"----"shop_name": "淘宝国泰永康大药房旗舰店";
...
//					tempMap.put("totalResults", totalResults);
			...
			logger.error("执行JD分页获取信息失败："+e1.getMessage(),e1);
	...
	public Map<String,Object> saveTradesSoldGetResponse6667(String tradesSoldGetResponse,int flag_save_update,Long multiChannelOrderBatch) {
		...
		try {
			...
			if (StringUtil.isStringNotBlank(success)) {
				...
				int recordNum=channelMapper.isExistChannelOrderBatch_JD(multiChannelOrderBatch);
				if (flag_save_update!=2 && recordNum<=0) {
					...
					multiChannelOrderBatchModel.setMultiChannelId(2);//看表ec_multi_channel，1:天猫
...
//		        		logger.info("更新订单状态-get_time-editTime");
				        ...
				        String tid_JD = "JD" +trades_JSONObject.getString("out_tid");//外部平台单号
				        orderModel.setOrderStatus(orderStatus_temp);
				        orderModel.setTid(tid_e);
				    		...
				    		if (companyName.equals("申通")) {
					    		...
					    		if (logisticCompanyId!=0) {
									...
									if (StringUtil.isBlank(channel_code)) {
							    		...
							    		logisticChannelModel.setChannelType("JD");
...
//						    	  BigDecimal divideFee99=new BigDecimal(0.0000);//
							      ...
							      for(int k=0;k<tradeDetails_JSONArray.size();k++){  //遍历 jsonarray 数组，把每一个对象转成 json 对象
								        ...
								        orderItemModel.setOrderNo(tid_JD);
...
//										divideFee99=MathUtil.add(divideFee99, divideFee55);
										
//										String ferght222 = tradeDetails_JSONObject.getString("ferght");
//										deliveryFee=MathUtil.add(divideFee55, new BigDecimal(ferght222));
...
//							      BigDecimal skuFee=MathUtil.add(deliveryFee, activityDiscountFee);
	...
	//以下是支付日志的开始 ********************************************
							      if("Y".equals(orderModel.getIsPay())){
										...
										logModel.setBusinessId(tid_JD);//订单号
										//utc时间格式转换必须*1000
								      ...
								      if(!"0.00".equals(trades_JSONObject.getString("discount_fee")) && null != trades_JSONObject.getString("discount_fee") && !"".equals(trades_JSONObject.getString("discount_fee"))){
								    	  PaymentLogModel logModel2 = new PaymentLogModel();
											...
											logModel2.setBusinessId(tid_JD);//订单号 
											//utc时间格式转换必须*1000
...
//以下是支付日志的结束****************************************************
			...
			logger.error("JD组装数据失败,订单号："+orderModel.getOrderNo()+" ,异常信息"+e.getMessage(),e);
		}
		return null;
	 }		
...
//			logger.info("商品号："+goodNo+"  69码："+goodsNo_69+"该商品暂无信息！");
    ...
    public static void main(String[] args) {
    	String str = new Edb_JD_OrderServiceImpl().getTradesSoldGetResponse(1, 10, new String[]{"2016-04-14 00:00:00","2016-04-15 20:00:00"});
