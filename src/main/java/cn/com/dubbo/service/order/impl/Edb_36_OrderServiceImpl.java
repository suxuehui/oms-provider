...
import cn.com.dubbo.service.order.Edb_36_OrderService;
...
//从e店宝同步360的订单
@Service("edb36OrderService")
//@Transactional
public class Edb_36_OrderServiceImpl implements Edb_36_OrderService{	
	...
	private static final Logger logger = Logger.getLogger(Edb_36_OrderServiceImpl.class);
	//--------------------------------------------------------------------------------------
...
//		apiparamsMap.put("end_time", "2016-07-06 13:38:40");
		...
		apiparamsMap.put("shopid", "4");//shopid:店铺id   
...
//					tempMap.put("totalResults", totalResults);
			...
			logger.error("执行360分页获取信息失败："+e1.getMessage(),e1);
	...
	public Map<String,Object> saveTradesSoldGetResponse6667(String tradesSoldGetResponse,int flag_save_update,Long multiChannelOrderBatch) {
		...
		try {
			...
			if (StringUtil.isStringNotBlank(success)) {
				...
				int recordNum=channelMapper.isExistChannelOrderBatch_36(multiChannelOrderBatch);
				if (flag_save_update!=2 && recordNum<=0) {
					...
					multiChannelOrderBatchModel.setMultiChannelId(3);//看表ec_multi_channel，1:天猫
...
//		        		logger.info("更新订单状态-get_time-editTime");
				        ...
				        String tid_36 = "36" +trades_JSONObject.getString("out_tid");//外部平台单号
				        orderModel.setOrderStatus(orderStatus_temp);
				        orderModel.setTid(tid_e);
...
//						    	  BigDecimal divideFee99=new BigDecimal(0.0000);//
							      ...
							      for(int k=0;k<tradeDetails_JSONArray.size();k++){  //遍历 jsonarray 数组，把每一个对象转成 json 对象
								        ...
								        orderItemModel.setOrderNo(tid_36);
	...
	//以下是支付日志的开始 ********************************************
							      if("Y".equals(orderModel.getIsPay())){
								      //货到付款金额处理
										...
										logModel.setBusinessId(tid_36);//订单号
										//utc时间格式转换必须*1000
								      ...
								      if(!"0.00".equals(trades_JSONObject.getString("discount_fee")) && null != trades_JSONObject.getString("discount_fee") && !"".equals(trades_JSONObject.getString("discount_fee"))){
								    	  PaymentLogModel logModel2 = new PaymentLogModel();
											...
											logModel2.setBusinessId(tid_36);//订单号 
											//utc时间格式转换必须*1000
...
//以下是支付日志的结束****************************************************
			...
			logger.error("360组装数据失败,订单号："+orderModel.getOrderNo()+" ,异常信息"+e.getMessage(),e);
		}
		return null;
	 }		
...
//			logger.info("商品号："+goodNo+"  69码："+goodsNo_69+"该商品暂无信息！");
    ...
    public static void main(String[] args) {
    	String str = new Edb_36_OrderServiceImpl().getTradesSoldGetResponse(1, 10, new String[]{"2016-04-01 00:00:00","2016-05-01 20:00:00"});
